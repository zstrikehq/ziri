...
- `master-key.txt` - Master key (if saved)
...
Host, port, mode, public URL, email settings. See [Config File](/configuration/config-file) for details.

### Environment Variables

Quick overrides for common settings. See [Environment Variables](/configuration/environment-variables) for details.


### Email

SMTP or SendGrid configuration. See [Email Setup](/configuration/email-setup) for details.
...
Use `PUT /api/config` to update configuration programmatically.
...
- [Environment Variables](/configuration/environment-variables) - Quick reference
- [Config File](/configuration/config-file) - Detailed config structure
- [Email Setup](/configuration/email-setup) - Email configuration
