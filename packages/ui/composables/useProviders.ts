...
export function useProviders() {
  ...
  const listProviders = async (params?: {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to list providers: ${response.statusText}`)
      }
  ...
  const addProvider = async (input: CreateProviderInput) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || `Failed to add provider: ${response.statusText}`)
      }
  ...
  const updateProvider = async (name: string, input: { apiKey?: string; displayName?: string }) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || `Failed to update provider: ${response.statusText}`)
      }
  ...
  const removeProvider = async (name: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || `Failed to remove provider: ${response.statusText}`)
      }
  ...
  const testProvider = async (name: string): Promise<{ status: string; models?: number }> => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.message || `Failed to test provider: ${response.statusText}`)
      }
